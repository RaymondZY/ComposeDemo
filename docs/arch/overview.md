...
:scaffold:platform            — Android/Compose 绑定层
...
:biz:xxx:core              — State / Event / Effect / UseCase（纯 Kotlin，零 Android 依赖）
:biz:xxx:platform        — ViewModel + Composable + Koin DI Module（平台相关）
```
...
| 类型       | 模块                                                | 说明                                | 测试方式                  |
|----------|---------------------------------------------------|-----------------------------------|-----------------------|
| **平台无关** | `:scaffold:core`、`:biz:*:core`、`:service:*:api` | 纯 Kotlin，零 Android 依赖             | JUnit（JVM 快速运行）       |
| **平台相关** | `:scaffold:platform`、`:biz:*:platform`、`:app`  | 依赖 AndroidX / Compose / ViewModel | JUnit + `androidTest` |
...
| `:scaffold:platform`   | ViewModel 生命周期、Compose 作用域、Koin 集成    | 仅依赖 AndroidX + `:scaffold:core`                                |
| `:service:*`          | 应用声明周期的服务                             | 可依赖 `:scaffold:core`                                           |
| `:biz:*:core`       | 业务逻辑：UseCase、State、Event、Effect（平台无关） | 仅依赖 `:scaffold:core` + `:service:*:api`                        |
| `:biz:*:platform` | UI 层：ViewModel、Composable、DI 模块（平台相关） | 可依赖 AndroidX + Compose + `:scaffold:platform` + `:biz:*:core` |
| `:app`                | 应用组装：Koin 启动、模块注册、Activity            | 依赖所有 `:biz:*:platform`                                     |
...
| `BaseViewModel`   | ViewModel 生命周期桥接，持有 StateHolder                  | `:scaffold:platform` |
| `MviScreen`       | Screen 级 Composable 作用域，创建 Koin Scope + Registry | `:scaffold:platform` |
